...
The dashboard Voice button launches the AI-guided freestyle talk. It skips the fixed acoustic prompts and starts directly with a short conversational AI question. The app speaks, listens to the user's response, transcribes it with FluidAudio Parakeet EOU streaming ASR by default, waits until the timed response window ends or the user taps Send Now, asks `/ai/voice-chat-turn` for the next short reply, and repeats for a 2-3 minute, four-turn conversation. Provider keys stay in `.env`; the iOS app only receives `VITALSCORE_AI_DIALOG_ENDPOINT`, provider, model config, voice transcription provider, speech-recognition mode, realtime transcription model, and optional voice identifier.

Live conversation prompts, transcripts, summaries, and debug WAV pointers are not retained in local app storage. Completed live sessions keep only the acoustic quality metrics, voice score, and baseline metadata needed for local trends; any legacy saved live conversation exports are scrubbed on launch.

The fixed prompt sequence still exists as the non-live voice-check mode for acoustic validation: quiet calibration, first sustained vowel, repeated sustained vowel, counting from 1 to 10, and the read-aloud sentence. The prompt text is shown on screen before each recording. Audio is analyzed on device for acoustic features, and raw audio is not stored unless the debug-only WAV export is explicitly enabled.

Live transcripts default to `VITALSCORE_VOICE_TRANSCRIPTION_PROVIDER=fluid_audio`, which uses FluidAudio's local Parakeet EOU streaming recognizer with 320 ms chunks and finalizes each completed voice turn with `finish()`. The first launch may need to download/cache the FluidAudio Core ML model assets; transcription then runs locally. Set `VITALSCORE_VOICE_TRANSCRIPTION_PROVIDER=apple_speech` to force Apple Speech, or leave FluidAudio enabled and the app will fall back to Apple Speech if the local provider cannot start. `VITALSCORE_SPEECH_RECOGNITION_MODE=best_available` controls that Apple fallback; set it to `on_device` when the priority is keeping Apple Speech local on supported devices. The server-side realtime transcription model default is `VITALSCORE_AI_TRANSCRIPTION_MODEL=gpt-realtime-whisper`; the local server exposes `/ai/realtime-transcription-session` to mint short-lived OpenAI Realtime transcription sessions without compiling the OpenAI API key into the app. Use `VITALSCORE_AI_TRANSCRIPTION_LANGUAGE=en` and `VITALSCORE_AI_TRANSCRIPTION_DELAY=low` unless you need a different language or want to trade more latency for higher transcript stability.
...
For true OpenAI live speech-to-text, the expected architecture is: iOS requests an ephemeral session from `/ai/realtime-transcription-session`, connects to OpenAI Realtime with that short-lived client secret, streams 24 kHz mono PCM, then consumes transcript delta/completion events. Keep `gpt-realtime-whisper` as the first-choice live transcription model; use `gpt-4o-transcribe` for high-accuracy non-realtime or file-style transcription, and `gpt-4o-mini-transcribe` only when cost and speed matter more than accuracy.
